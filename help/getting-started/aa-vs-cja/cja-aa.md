...
Las siguientes tablas enumeran qué funciones de Adobe Analytics (AA) son compatibles, parcialmente compatibles o no compatibles con el Customer Journey Analytics (CJA) y qué características de CJA no son compatibles o no están disponibles en AA. Estas listas cambiarán con el tiempo a medida que se añadan funciones a CJA.
...
| --- | --- |
| Alojamiento para cualquier tipo de datos | CJA se combina con la capacidad de Experience Platform de albergar todo tipo de esquemas y tipos de datos. Uso [Modelo de datos de experiencia (XDM)](https://experienceleague.adobe.com/docs/experience-platform/xdm/home.html?lang=es), los datos se pueden representar y organizar de forma uniforme, listos para la combinación y exploración. Adobe Analytics se centra principalmente en los datos de análisis web y móviles, con algunas capacidades para [importar datos](https://experienceleague.adobe.com/docs/analytics/import/home.html?lang=es). |
| Métricas y Dimension de clientes ilimitados | Las dimensiones de CJA son ilimitadas; los valores pueden ser numéricos, de texto, de objetos, de listas o de mezclas de todos. Los Dimension pueden estar anidados o jerárquicos. Analytics admite hasta un máximo de 75 props y 250 eVars. Esto elimina las limitaciones de medición actuales mediante dimensiones y eventos. |
| Cardinalidad ilimitada / valores únicos | CJA admite valores únicos ilimitados o elementos de dimensión que se pueden registrar en una sola dimensión. AA está limitada a valores únicos de 500.000. Esto elimina las limitaciones de informes y análisis que existen actualmente con la implementación a gran escala de Analytics. |
| Transformaciones de tiempo de los informes | Las transformaciones de tiempo de los informes (más conocidas como Vistas de datos) en CJA le permiten interpretar aún más los datos de una conexión. Puede modificar o eliminar datos sin necesidad de volver a implementarlos; utilizar subcadenas para manipular dimensiones; crear métricas a partir de cualquier valor; filtrar subeventos. Y todo esto puede hacerse de manera no destructiva. Adobe Analytics proporciona funciones limitadas mediante grupos de informes virtuales y sesiones. |
| Análisis de experimentación | CJA puede evaluar el alza y la confianza de cualquier experimento desde cualquier fuente de datos definida como parte de una conexión. Esto le permite comprender las relaciones de causa y efecto entre las interacciones de los clientes que abarcan cualquier canal. Analytics se limita a los análisis de experimentación mediante la integración de Analytics for Target (A4T). |
| Análisis entre dispositivos | CJA admite la combinación perfecta de conjuntos de datos específicos del dispositivo de sesiones no autenticadas y autenticadas. También puede rellenar datos históricos con dispositivos conocidos. En Analytics, esta funcionalidad se limita a un único grupo de informes y al uso de un gráfico de dispositivos. |
| Acceso SQL | Con la opción Distiller de datos , CJA puede eliminar las limitaciones de los datos recopilados en el procesamiento back-end de Adobe. Puede modificar los datos con SQL, crear nuevos valores y conjuntos de datos exclusivos de su negocio y seguir explorando. Analytics no admite ningún tipo de acceso SQL a sus datos. |
| Opciones de seguridad y privacidad mejoradas: preparación para HIPAA | CJA está lista para HIPAA y ofrece opciones de seguridad adicionales para el cumplimiento de la normativa. Adobe Analytics no está preparado para HIPAA. |
