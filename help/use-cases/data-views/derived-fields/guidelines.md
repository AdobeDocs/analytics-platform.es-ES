...
Los [campos derivados](/help/data-views/derived-fields/derived-fields.md) de Customer Journey Analytics le permiten transformar, clasificar y enriquecer datos en el momento de la consulta sin modificar los conjuntos de datos de origen. Esa flexibilidad puede introducir complejidad, problemas de rendimiento y sobrecarga de mantenimiento si se aplica sin disciplina.
...
* [Campos derivados de alta cardinalidad](#high-cardinality-derived-fields)
* [Caso demasiado complejo al encadenar reglas](#over-complex-case-when-rule-chains)
* [Uso incorrecto](#wrong-usage)
* [Clasificaciones erróneas de métricas y dimensiones](#misclassifications-of-metrics-and-dimensions)
* [Dificultades del canal de marketing y de la lógica de campaña](#marketing-channel-and-campaign-logic-pitfalls)
* [Claves de cadena no normalizadas utilizadas en búsquedas](#non-normalized-string-keys-used-in-lookups)
* [Uso indebido o extralimitado de regex](#regex-misuse-or-overreach)
* [Lógica de estilo de métrica calculada en campos derivados](#calculated-metric-style-logic-in-derived-fields)
* [Uso excesivo de las funciones Siguiente o Anterior o secuencial](#over-usage-of-next-or-previous-or-sequential-functions)
* [Ignorar contexto de nivel de persona y sesión](#ignoring-session-and-person-level-context)
* [Alcanzar o acercarse a los límites de funciones documentadas](#hitting-or-nearing-documented-function-limits)
* [Reglas de optimización específicas de la vista de datos](#data-view-specific-optimization-rules)
...
* Las operaciones simples como [Minúsculas](/help/data-views/derived-fields/derived-fields.md#lowercase), [Recortar](/help/data-views/derived-fields/derived-fields.md#trim) o [Mayúsculas y minúsculas cuando](/help/data-views/derived-fields/derived-fields.md#case-when) comprueba la dirección URL de la página suelen ser más costosas que la misma lógica en los campos de baja cardinalidad.
...
* Evite hacer referencia a direcciones URL de página completa o a componentes de alta cardinalidad similares directamente en los segmentos predeterminados de vistas de datos. Inserte una lógica de URL pesada ([Case When](/help/data-views/derived-fields/derived-fields.md#case-when), [Regex Replace](/help/data-views/derived-fields/derived-fields.md#regex-replace), varias funciones de cadena) en sentido ascendente a [preparación de datos](https://experienceleague.adobe.com/es/docs/experience-platform/data-prep/home) o [conjuntos de datos de consulta](/help/getting-started/cja-upgrade/cja-upgrade-dataset-lookup.md), de modo que las clasificaciones resultantes se dirijan a dimensiones más sencillas y de baja cardinalidad.
...
En esta sección se describen las cadenas más complejas de [reglas Case When](/help/data-views/derived-fields/derived-fields.md#case-when).

Customer Journey Analytics aplica [límites explícitos de funciones y operadores](/help/data-views/derived-fields/derived-fields.md#limitations) por campo derivado (por ejemplo, número máximo de operadores, número máximo de funciones por tipo). Las funciones y cadenas demasiado complejas dentro de las funciones son más difíciles de mantener y más propensas a errores.

**Patrones**

* [Caso muy grande When](/help/data-views/derived-fields/derived-fields.md#case-when) funciona con cadenas **[!UICONTROL If]** y **[!UICONTROL Else If]** complejas:
   * Muchas condiciones (por ejemplo: más de 20 operadores) o anidamiento profundo (más de 3 o 4 niveles de [Case When](/help/data-views/derived-fields/derived-fields.md#case-when) **[!UICONTROL If]** y **[!UICONTROL Else If]** anidados).
...
* Rendimiento potencial y riesgo límite: puede alcanzar o acercarse a [límites de operador o función](/help/data-views/derived-fields/derived-fields.md#limitations), especialmente con patrones de clasificación.
...
* Un campo derivado replica el comportamiento ya disponible en la configuración del componente:
   ...
   * Lógica de persistencia o atribución codificada con [Siguiente o Anterior](/help/data-views/derived-fields/derived-fields.md#next-or-previous) o lógica de secuencia manual donde la configuración de la vista de datos [atribución](/help/data-views/component-settings/attribution.md) y [caducidad](/help/data-views/component-settings/persistence.md) sería suficiente.
   * Una métrica derivada que simplemente cuenta una métrica existente bajo una condición.
     ...
     Este enfoque replica lo que podría lograr una métrica filtrada o [Incluir valores de exclusión](/help/data-views/component-settings/include-exclude-values.md).
...
* Atribución y persistencia: use la configuración de la vista de datos [Persistencia](/help/data-views/component-settings/persistence.md) (**[!UICONTROL Modelo de asignación]** y **[!UICONTROL Caducidad]**) para las dimensiones en lugar de simularlas en un campo derivado con [Siguiente o Anterior](/help/data-views/derived-fields/derived-fields.md#next-or-previous) u otra lógica secuencial.
* Agrupación numérica: mantenga el campo derivado numérico y permita que la vista de datos cree una dimensión agrupada en la parte superior, en lugar de programar etiquetas de intervalo en una cadena [Case When](/help/data-views/derived-fields/derived-fields.md#case-when).
...
* Una función [Lookup](/help/data-views/derived-fields/derived-fields.md#lookup) sobre un evento o campo de perfil que alimenta un conjunto de datos de búsqueda.
* No hay [Minúsculas](/help/data-views/derived-fields/derived-fields.md#lowercase), [Recortar](/help/data-views/derived-fields/derived-fields.md#trim) o [Reemplazo de regex](/help/data-views/derived-fields/derived-fields.md#regex-replace) anteriores que estandarizan la clave.
...
* Agregue las funciones [Minúsculas](/help/data-views/derived-fields/derived-fields.md#lowercase) y [Recortar](/help/data-views/derived-fields/derived-fields.md#trim) antes de la función [Buscar](/help/data-views/derived-fields/derived-fields.md#lookup) a menos que haya una razón documentada para conservar las mayúsculas o minúsculas.
...
* [Regex Replace](/help/data-views/derived-fields/derived-fields.md#regex-replace) o condiciones basadas en regex usan patrones amplios; las funciones [Case When](/help/data-views/derived-fields/derived-fields.md#case-when) más sencillas con **[!UICONTROL Contains]** o **[!UICONTROL Starts with]** son mejores alternativas.
...
* Uso de regex para analizar direcciones URL en lugar de usar la función [Análisis de URL](/help/data-views/derived-fields/derived-fields.md#url-parse).
...
* Prefiera [Análisis de URL](/help/data-views/derived-fields/derived-fields.md#url-parse) para elementos de URL estándar (dominio, ruta, parámetros de consulta) en lugar de [Reemplazo de Regex](/help/data-views/derived-fields/derived-fields.md#regex-replace).
* Para las comprobaciones de patrones simples, use la lógica [Case When](/help/data-views/derived-fields/derived-fields.md#case-when) with **[!UICONTROL Contains]**, **[!UICONTROL Starts with]** o **[!UICONTROL Ends with]** en lugar de expresiones regulares con [Regex Replace](/help/data-views/derived-fields/derived-fields.md#regex-replace).
...
En esta sección se describe el uso excesivo de [Siguiente o Anterior](/help/data-views/derived-fields/derived-fields.md#next-or-previous) o funciones secuenciales.

**Patrones**

* Un campo derivado usa las funciones [Siguiente o Anterior](/help/data-views/derived-fields/derived-fields.md#next-or-previous) varias veces (cerca del límite documentado por campo).
* [Siguiente o Anterior](/help/data-views/derived-fields/derived-fields.md#next-or-previous) se usa para implementar lógica de persistencia (por ejemplo: llevar una campaña hacia adelante) en lugar de usar persistencia de vista de datos.
...
* Redundancia con persistencia de dimensión: la configuración de la vista de datos [Persistencia](/help/data-views/component-settings/persistence.md) (modelo de asignación) en la dimensión cubre mejor algunos casos de uso (por ejemplo, Canal de último contacto en una sesión).

**Recomendaciones**

* Para patrones que se asemejan a la persistencia estándar (por ejemplo, llevar un valor hacia adelante en una sesión o persona), use la configuración de [Persistencia](/help/data-views/component-settings/persistence.md) de la dimensión (**[!UICONTROL Modelo de asignación]** y **[!UICONTROL Caducidad]**) en la vista de datos en lugar de simular estos patrones con [Siguiente o Anterior](/help/data-views/derived-fields/derived-fields.md#next-or-previous).
* Reserve [Siguiente o Anterior](/help/data-views/derived-fields/derived-fields.md#next-or-previous) para una ruta de varios pasos avanzada o un etiquetado funnel que la persistencia de la dimensión por sí sola no pueda lograr (por ejemplo: concatenación de secuencia de canal).
...
>[!NOTE]
>
>Reduzca la dependencia en campos de alta cardinalidad dentro de campos derivados complejos siempre que sea posible (por ejemplo: use claves normalizadas o clasificaciones agrupadas) para limitar el costo de la consulta y la probabilidad de alcanzar [límites de operador o función](/help/data-views/derived-fields/derived-fields.md#limitations).

[documentos](/help/data-views/derived-fields/derived-fields.md#limitations) de Customer Recorrido Analytics para funciones y operadores máximos por campo derivado, incluidos los límites por tipo de función.patrones**

* Un campo derivado usa muchas operaciones [Lookup](/help/data-views/derived-fields/derived-fields.md#lookup), [Math](/help/data-views/derived-fields/derived-fields.md#math), [Split](/help/data-views/derived-fields/derived-fields.md#split) u otras funciones.
* El número de operadores está cerca de los [límites documentados](/help/data-views/derived-fields/derived-fields.md#limitations) (por ejemplo: más del 70% - 80% de los recuentos permitidos).
