...
1. Especifique la siguiente información básica para configurar el lienzo de Recorrido:
   ...
   | Campo | Función |
   |---------|----------|
   | [!UICONTROL **Métrica principal**] | Determina el Métrica que se utiliza al calcular los valores porcentuales y numéricos de cada nodo del recorrido.<p>**Nota**: La ámbito de los datos incluidos en cada valor de porcentaje y número viene determinada por la Métrica que elija en el **[!UICONTROL campo de contenedor]** del lienzo Recorrido. Por ejemplo, si **[!UICONTROL Persona]** se establece como el contenedor, las estadísticas que se muestran en el viaje abarcan varias sesiones para una persona determinada. Si **[!UICONTROL Sesión]** se establece como el contenedor, las estadísticas que se muestran en el viaje se limitan a una sola sesión definida para una persona determinada.</p><p>Considere los siguientes ejemplos de cómo el Métrica principal afecta los valores porcentuales y numéricos de cada nodo:</p><ul><li>Si _Personas_ es el Métrica principal y _la Persona_ es el contenedor, entonces solo aquellas personas que tienen un evento que coincide con los criterios de cada nodo sucesivo en el viaje se mueven a lo largo del viaje. Las consecuencias ocurren en un nodo cuando una persona nunca llegó a ninguno de los siguientes nodos inmediatos en el viaje. Es posible que hayan realizado otras acciones en el sitio, pero no cumplieron con los criterios definidos por ninguno de los nodos que seguir inmediatamente.</li><li>Si _Personas_ es la métrica principal y _Sesión_ es el contenedor, solo se moverán por el recorrido aquellas personas que tengan un evento que coincida con los criterios de cada nodo en el recorrido dentro de una sola sesión. Las visitas en el orden previsto se producen en un nodo cuando una persona nunca llegó a ninguno de los nodos inmediatamente siguientes del recorrido en una sola sesión. Es posible que hayan realizado otras acciones en el sitio dentro de la sesión, pero no cumplieron los criterios definidos por ninguno de los nodos que siguen inmediatamente.</li></ul> <p>La Métrica principal afecta a los siguientes aspectos de la visualización del lienzo del recorrido:</p><ul><li>El número total que se muestra en cada nodo.  <p>Por ejemplo, si Eventos es el Métrica principal, cada nodo muestra el número de personas que tuvieron un evento que coincide con los criterios de ese nodo (y cada nodo previo a él en el viaje).</p></li><li>El porcentaje que se muestra en cada nodo. (Una vez creada la visualización, puede usar el menú desplegable **[!UICONTROL Valor de porcentaje]** para elegir mostrar el porcentaje del total, el porcentaje del nodo anterior o el porcentaje del nodo de inicio).<p>Por ejemplo, si Eventos es la métrica principal, cada nodo muestra el porcentaje de personas que tuvieron un evento que coincide con los criterios de ese nodo (y con cada nodo anterior que lo precedió en el recorrido).</p></li><li>Cuando se añade una dimensión a la visualización, se añaden los 3 nodos principales de la visualización en función de la métrica principal.</li></ul> |
   | [!UICONTROL **Métrica secundaria**] | Determina la métrica secundaria que se usa al calcular los valores porcentuales y numéricos en cada nodo del recorrido. La métrica secundaria es opcional. <p>**Nota**: el ámbito de los datos incluidos en cada valor porcentual y numérico viene determinado por la métrica que elija en el campo **[!UICONTROL contenedor de lienzo de Recorrido]**. Por ejemplo, si **[!UICONTROL Persona]** se establece como contenedor, las estadísticas que se muestran en el recorrido abarcan varias sesiones para una persona determinada. Si **[!UICONTROL Session]** se establece como contenedor, las estadísticas que se muestran en el recorrido se limitan a una sola sesión definida para una persona determinada.</p><p>Cuando se configura una métrica secundaria, afecta a los siguientes aspectos de la visualización del lienzo de Recorrido:</p><ul><li>El número total que se muestra en cada nodo debajo de la métrica principal. <p>Por ejemplo, si Cuentas es la métrica secundaria, el número de cuentas se muestra en el nodo para todas las personas que llegaron a ese nodo en el recorrido.</p></li><li>El porcentaje que se muestra en cada nodo debajo de la métrica principal. (Una vez creada la visualización, puede elegir mostrar el porcentaje del total o del nodo de inicio).</li><p>Por ejemplo, si Sesiones es la métrica secundaria, cada nodo muestra el porcentaje de sesiones que alcanzaron ese nodo en el recorrido (el porcentaje del total o del nodo de inicio).</p></li></ul> |
...
1. Seleccione [!UICONTROL **Generar**].
   ...
   Si seleccionó un recorrido de Journey Optimizer, el recorrido se muestra con el mismo orden, secuencia y estructura que en Journey Optimizer. (Solo los usuarios con acceso al optimizador de Recorrido pueden seleccionar un recorrido de Journey Optimizer).

   <!-- add screen shot -->

   Si no ha seleccionado ningún recorrido de Journey Optimizer, aparece un lienzo en blanco en el que puede empezar a añadir nodos al recorrido. (Solo los usuarios con acceso al optimizador de Recorrido pueden seleccionar un recorrido de Journey Optimizer).
...
1. Configure cualquiera de las siguientes opciones que se muestran en la parte superior de la visualización:
   ...
   |---------|----------|
   | [!UICONTROL **Valor porcentual**] | El valor porcentual que se muestra en cada nodo del recorrido.<p>![valor porcentual](assets/journey-canvas-percentage.png)</p> <p>Tenga en cuenta lo siguiente al configurar los valores porcentuales que se muestran en los nodos del recorrido:</p><ul><li>Se muestra un porcentaje en cada nodo del Métrica principal. También se muestra un porcentaje para el Métrica secundario si se ha configurado uno. (Para obtener más información sobre la configuración del Métrica principal y secundario, consulte [Comience a crear una visualización de lienzo de](#begin-building-a-journey-canvas-visualization) recorrido).</li><li>Los porcentajes incluyen a todas las personas o sesiones que están incluidas en los datos vista dentro del intervalo de fecha del panel. El uso de personas _o_ sesiones __ depende de la configuración contenedor. (Para obtener más información sobre la configuración contenedor, consulte [Comience a crear una visualización de lienzo de](#begin-building-a-journey-canvas-visualization) recorrido).</li></ul> <p>Elija entre las siguientes opciones:</p> <ul><li>[!UICONTROL **Porcentaje del nodo de inicio**]: calcula los porcentajes mostrados en cada nodo en relación con el nodo de inicio. Los porcentajes se basan en la métrica principal y secundaria que haya seleccionado. <p>Un _nodo de inicio_ es un nodo que no tiene nodos conectados antes de él.</p><p>Un recorrido puede contener varios nodos inicio. Sin embargo, [!UICONTROL **se utiliza el porcentaje del total**] si el viaje contiene 2 o más nodos de inicio que posible cliente a un nodo común. Si desea utilizar [!UICONTROL **el Porcentaje de inicio nodo**], actualice el recorrido para que cada nodo del viaje se pueda rastrear hasta una sola inicio nodo.</p></li><li>[!UICONTROL **Porcentaje de nodo**] anterior: calcula los porcentajes mostrados en cada nodo en relación con el nodo anterior. Los porcentajes se basan en la métrica principal y secundaria que haya seleccionado.</li><li>[!UICONTROL **Porcentaje del total**]: calcula los porcentajes mostrados en cada nodo en relación con todos los datos de la vista de datos. Los porcentajes se basan en la métrica principal y secundaria que haya seleccionado.</li></ul> |
...
1. Arrastre métricas, dimensiones, elementos de dimensión, filtros o intervalos de fechas desde el carril izquierdo al lienzo. Se admiten las métricas basadas en un [campo derivado](/help/data-views/derived-fields/derived-fields.md). Sin embargo, no se admiten las métricas calculadas ni las métricas o dimensiones basadas en un [conjunto de datos de resumen](/help/data-views/summary-data.md).
   ...
   La visualización se actualiza en función de la métrica principal de la siguiente manera (según el tipo de componente y el área del lienzo donde se coloque):

   | Tipo de componente | Colocación del componente | La visualización se actualiza después de agregar nodo |
   |---------|----------|----------|
   ...
   | Múltiples componentes | Un área en blanco del lienzo | **Si ninguno de los componentes es una dimensión:**<p>Cada componente se muestra como un nodo independiente donde los componentes se soltaron, sin conexión con ningún nodo existente.</p><p>Mantenga presionada la tecla Mayús cuando suelte los componentes en el lienzo para agregarlos como una nodo combinada. </p><p>**Si alguno de los componentes que está agregando son dimensiones:**</p><p>Cada componente se muestra como un nodo independiente donde se soltaron los componentes, sin conexión con ningún nodo existente.</p><p>Solo se puede añadir una dimensión a la vez. Cuando se añade la dimensión, se crean 3 nodos para los 3 elementos de dimensión principales en los que se soltó el componente.</p><p>Mantenga pulsada la tecla Mayús cuando suelte los componentes en el lienzo para añadirlos como un nodo combinado. Los 3 elementos de dimensión principales se combinan con cada nodo. (Consulte [Combinar nodos](#combine-nodes) para obtener más información).</p> |
   ...
   | Múltiples componentes | Una flecha que conecta 2 nodos existentes | **Si ninguno de los componentes es una dimensión:**<p>Cada componente se muestra como un nodo independiente donde los componentes se soltaron y cada nodo está conectado a ambos nodos existentes. (Consulte [Conectar nodos](#connect-nodes) para obtener más información).</p><p>Mantenga pulsada la tecla Mayús cuando suelte los componentes en el lienzo para añadirlos como un nodo combinado. (Los componentes deben ser del mismo tipo para combinarse en un solo nodo). (Vea [Combinar nodos](#combine-nodes) para obtener más información).</p><p>**Si alguno de los componentes que está agregando son dimensiones:**</p><p>Cada componente se muestra como un nodo independiente donde los componentes se soltaron y cada nodo está conectado a ambos nodos existentes.</p><p>Solo se puede añadir una dimensión a la vez. Cuando se agrega la dimensión, se crean 3 nodos para los 3 elementos principales de la dimensión que siguen al primer evento después del primer nodo (de personas o sesiones que finalmente llegan al segundo nodo). Cada nodo está conectado a los dos nodos existentes. (Consulte [Conectar nodos](#connect-nodes) para obtener más información).</p><p>Mantenga pulsada la tecla Mayús cuando suelte los componentes en el lienzo para añadirlos como un nodo combinado. Los 3 elementos de dimensión principales se combinan con cada nodo y cada nodo está conectado a los dos nodos existentes. (Consulte [Combinar nodos](#combine-nodes) para obtener más información).</p> |
...
Puede mostrar automáticamente los nodos inmediatos principales en función de los nodos que ya están en el lienzo. Puede agregar los nodos principales al lienzo de Recorrido o verlos en una tabla de forma libre.
...
Puede seleccionar un nodo y mostrar los elementos principales dimensión que vienen inmediatamente antes en el viaje. Puede agregar los 3 elementos de dimensión principales al lienzo de Journey como nodos separados, o puede vista todos los elementos de dimensión superior en una tabla de forma libre.

1. Haga clic con el botón derecho en la nodo donde desee mostrar los elementos principales dimensión que vienen antes en el viaje.
...
1. Seleccione la dimensión que desee en la lista de dimensiones.
   ...
   Dependiendo de lo que haya elegido en el paso anterior, los 3 elementos de dimensión principales se agregan al lienzo como 3 nodos separados, o todos los elementos de dimensión superior se muestran en una tabla de forma libre.
...
| Filtrar + Métrica, intervalo de fechas o Dimension | Junto con Y |
...
* **Anchura**: indica el porcentaje de volumen de una nodo a otra
...
* Los desgloses se aplican al Métrica primario. El Métrica secundario no se ve afectado.

* Aplicar un desglose no cambia el viaje. Más bien, simplemente muestra una desglose de los datos para el nodo donde se aplica.

* Si una nodo ya tiene una desglose, la aplicación de una nueva desglose reemplaza a la existente.
...
1. En una visualización del lienzo del Recorrido, seleccione uno o varios nodos en los que desee aplicar un desglose y, a continuación, haga clic con el botón derecho en uno de los nodos seleccionados.
   ...
   Para seleccionar varios nodos o flechas, mantenga pulsada la tecla Comando (en Mac) o Ctrl (en Windows).
...
1. En una visualización de lienzo de recorrido, haga clic con el botón derecho en la nodo o flecha que desee usar para crear el filtro.

1. Seleccione [!UICONTROL **Crear filtro de nodo**] o [!UICONTROL **Crear filtro de flecha**].

   Se muestra el generador de Filtrar. En la [!UICONTROL **sección Definición**] , la definición del filtro se crea en función de la nodo o flecha seleccionada y su contexto dentro del recorrido.

1. Especifique un título para el filtro y realice los demás cambios. Para obtener más información sobre cómo crear un filtro, consulte [Generador de filtros](/help/components/filters/filter-builder.md).
