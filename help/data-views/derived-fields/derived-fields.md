...
>[!NOTE]
>
>[Los componentes estándar](../component-reference.md) están vinculados y asociados a conjuntos de datos de eventos. Como resultado, los componentes estándar que se utilizan como parte de un campo derivado solo funcionan con datos de un conjunto de datos de evento.
>
...
Los componentes estándar solo se admiten en conjuntos de datos de eventos en campos derivados.

>[!TIP]
>
>Consulte [directrices](/help/use-cases/data-views/derived-fields/guidelines.md) para ver prácticas recomendadas, protecciones y escollos comunes al trabajar con campos derivados.
>
...
Esta plantilla de funciones obtiene la latitud para un estado de los EE. UU. con una precisión de cinco dígitos.
...
![Captura de pantalla del generador de reglas de la latitud del estado](assets/function-template-state-latitude.png)
...
Esta plantilla de funciones obtiene la longitud de un estado de los EE. UU. con una precisión de cinco dígitos.
...
>title="Case When"
>abstract="Esta función proporciona la capacidad de aplicar condiciones basadas en criterios definidos de uno o varios campos. A continuación, estos criterios se utilizan para definir los valores del nuevo campo derivado en función de la secuencia de las condiciones."
...
Customer Journey Analytics utiliza una estructura de contenedor anidada, siguiendo el modelo de Adobe Experience Platform [XDM](https://experienceleague.adobe.com/docs/experience-platform/xdm/home.html?lang=es) (modelo de datos de experiencia). Consulte [Contenedores](../create-dataview.md#containers) y [Contenedores de segmentos](/help/components/segments/seg-overview.md#containers) para obtener más información de contexto. Este modelo de contenedor, aunque flexible por naturaleza, establece algunas restricciones para utilizar el generador de reglas.
...
Devuelve la diferencia entre dos fechas o dos campos de fecha-hora.
...
Como analista de marketing de una empresa hotelera, le gustaría conocer la diferencia del número de días entre las fechas de llegada y las fechas de reserva de los clientes a lo largo de la última semana.


### Campo derivado {#datemath-uc1-derivedfield}

Defina un campo derivado `Days between booking and check-in`. Utilice la función [!UICONTROL ARITMÉTICA DE FECHAS] para definir una regla que calcule los días del [!UICONTROL Ámbito] [!DNL Person] entre la [!UICONTROL Fecha de reserva] y la [!UICONTROL Fecha de entrada]. Seleccione [!UICONTROL Día] como [!UICONTROL Granularidad de salida]. Y seleccione [!UICONTROL Devolver el último] tanto para la [!UICONTROL Fecha de reserva] como para la [!UICONTROL Fecha de entrada] para garantizar que se use el valor de ámbito de la última persona en el cálculo.

![Captura de pantalla de la regla Aritmética de fechas](assets/datemath-1.png)


## Caso de uso 2 {#datemath-uc2}

Como analista de marketing de una tienda física, quiere saber cuántos días hace que un cliente visitó la tienda por última vez. La funcionalidad de geolocalización se utiliza en una aplicación móvil y las señalizaciones de la tienda para capturar las visitas físicas de los clientes.

### Campo derivado {#datemath-uc2-derivedfield}

Defina un campo derivado `Days Since Visit To Shop`. Utilice la función [!UICONTROL ARITMÉTICA DE FECHAS] para definir una regla que calcule los días entre una fecha y hora personalizada (que se especifica en [!UICONTROL Fecha]) y la [!UICONTROL Hora local] (de su grupo de campos [!UICONTROL placeContext] del conjunto de datos de evento) con un [!UICONTROL Ámbito de anulación de duplicación] de [!UICONTROL Persona]. Seleccione [!UICONTROL Devolver el último] para garantizar que se use el último valor de ámbito de persona de [!UICONTROL Hora local] en el cálculo. Seleccione Día como [!UICONTROL Granularidad de salida].

![Captura de pantalla de la regla Aritmética de fechas 2](assets/datemath-2.png)

También puede usar el valor práctico de intervalo de fecha dinámico Ahora para calcular entre ahora y la [!UICONTROL Hora local] (de su grupo de campos [!UICONTROL placeContext] de su conjunto de datos de evento)

![Captura de pantalla de la regla Aritmética de fechas 2a](assets/datemath-2a.png)
...
Las opciones para [!UICONTROL Devolver el primero] o [!UICONTROL Devolver el último] no están disponibles cuando selecciona un campo basado en personas (de un conjunto de datos de perfil). Un campo basado en personas solo puede tener un valor para un campo Fecha o Fecha-hora para una persona.
+++
...
Devuelve la profundidad de un campo, similar a lo que es posible con la [dimensión Profundidad de evento estándar](/help/components/dimensions/overview.md#standard-dimensions) predeterminada.
...
| Tipo de datos de entrada | Entrada | Operadores incluidos | Limitaciones | Salida |
|---|---|---|---|---|
| Cualquiera | Cualquier campo | N/D | 3 funciones por campo derivado | Nuevo campo derivado |
...
Defina un campo derivado `Internal Search Depth`. La función [!UICONTROL PROFUNDIDAD] se usa para definir una regla con el fin de recuperar la profundidad de [!UICONTROL Término de búsqueda interna] y almacenarla en un nuevo campo derivado.

![Captura de pantalla de la regla Profundidad](assets/depth-1.png)

A continuación, utilice ese nuevo campo derivado en una visualización para desglosar el término que se ha utilizado para buscar en la primera búsqueda.

![Captura de pantalla de la regla Profundidad](assets/depth-1a.png)
...
|---|---|---|---|---|
| Inicio | Inicio | búsqueda | *Ningún valor* | *Ningún valor* |
| Inicio | búsqueda | búsqueda | Inicio | *Ningún valor* |
...
| información del producto | búsqueda | búsqueda | búsqueda | búsqueda |
| búsqueda | búsqueda | *Ningún valor* | información del producto | información del producto |
| búsqueda | *Ningún valor* | *Ningún valor* | búsqueda | información del producto |
...
|---|---|---|---|---|
| Inicio | Inicio | búsqueda | *Ningún valor* | *Ningún valor* |
| Inicio | Inicio | búsqueda | Inicio | *Sin valor* |
| Inicio | búsqueda | búsqueda | Inicio | *Ningún valor* |
| búsqueda | búsqueda | información del producto | Inicio | Inicio |
|   |   |   |   |   |
| búsqueda | búsqueda | información del producto | búsqueda | Inicio |
| búsqueda | información del producto | información del producto | búsqueda | Inicio |
| información del producto | *Ningún valor* | *Ningún valor* | búsqueda | búsqueda |
|   |   |   |   |   |
...
Crea un campo derivado de `Responses`. Utilice la función [!UICONTROL DIVISIÓN] para definir una regla que use el método [!UICONTROL Convertir en matriz] para convertir los valores del campo [!UICONTROL Respuesta de la aplicación de voz] usando `,` como [!UICONTROL Delimitador].
...
Crea un campo derivado de `Product Identifier`. Utilice la función [!UICONTROL RECORTAR] para definir una regla para **[!UICONTROL Recortar espacio en blanco]** desde el campo **[!UICONTROL ID del producto]**.
...
Crea un campo derivado de `Store Identifier`. Utilice la función [!UICONTROL RECORTAR] para definir una regla para [!UICONTROL Truncar por la derecha] el campo [!UICONTROL storeID] desde el final de la cadena hasta la posición `3`.

![Captura de pantalla de la regla Dividir: primer valor](assets/trim-3.png)
...
Tiene un campo de entero, altura de pantalla (por ejemplo, device.screenHeight del conjunto de datos de evento), que desea utilizar como dimensión basada en cadenas.


### Campo derivado {#typecast-uc1-derivedfield}

Usted define un campo derivado de `Screen Height`. La función [!UICONTROL CONVERSIÓN DE TIPOS] se usa para definir una regla en [!UICONTROL Convertir tipo a] [!UICONTROL Cadena] en el campo [!UICONTROL Altura de campo] y almacenarla en el nuevo campo derivado.

![Captura de pantalla de la regla Conversión de tipos 1](assets/typecast-1.png)



## Caso de uso 2 {#typecast-uc2}

Desea utilizar Ingresos en una tabla de cohorte (que sólo admite números enteros), pero el campo Ingresos tiene un tipo Doble.

![Captura de pantalla de la regla Conversión de tipos 2](assets/typecast-2.png)


### Campo derivado {#typecast-uc2-derivedfield}

Usted define un campo derivado de `Revenue (integer)`. La función [!UICONTROL CONVERSIÓN DE TIPOS] se usa para definir una regla en [!UICONTROL Convertir tipo a] [!UICONTROL Entero] en el campo [!UICONTROL Ingresos] y almacenarla en el nuevo campo derivado.
